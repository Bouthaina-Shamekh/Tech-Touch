<x-dashboard-layout>
    ...
    <x-slot:breadcrumbs>
        ...
        <li class="breadcrumb-item"><a href="{{route('admin.about.index')}}">{{__('about')}}</a></li>
        <li class="breadcrumb-item" aria-current="page">{{__('Edit About')}}</li>
    </x-slot:breadcrumb>
                ...
                <div class="card-body">
                    <form action="{{route('admin.about.update',$abouts->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-6 mb-3">
                                <x-form.input name="title_ar" label="{{__('Title_AR')}}" type="text" placeholder="{{__('enter title of about in arabic')}}" required :value="$abouts->title_ar" />
                            </div>
                            <div class="form-group col-6 mb-3">
                                <x-form.input name="title_en" label="{{__('Title_EN')}}" type="text" placeholder="{{__('enter title of about in english')}}" required :value="$abouts->title_en"/>
                            </div>

                            <div class="form-group col-6 mb-3">
                                <label for="content_en" class="form-label">{{__('Content Arabic')}}</label>
                                <textarea name="description_ar" id="description_ar" rows="3" class="form-control" required>{{$abouts->description_ar}}</textarea>
                            </div>
                            <div class="form-group col-6 mb-3">
                                <label for="content_en" class="form-label">{{__('Content English')}}</label>
                                <textarea name="description_en" id="description_en" rows="3" class="form-control" required>{{$abouts->description_en}}</textarea>
                            </div>
                            ...
                            <div class="form-group col-6">
                                ...
                                <img src="{{asset('storage/' . $abouts->image)}}" alt="img...." width="100px" class="mt-3">
                            </div>
                        </div>
                        <div class="row justify-content-end mt-3">
                            <a href="{{route('admin.about.index')}}" class="btn btn-secondary col-1 mr-3">
                                {{__('Back')}}
                            </a>
