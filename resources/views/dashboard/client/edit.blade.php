                ...
                <div class="card-body">
                    <form action="{{route('admin.client.update',$clients->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-6 mb-3">
                                <x-form.input name="stars" label="{{__('Star')}}" type="text" placeholder="{{__('enter of star')}}" required :value="$clients->stars"/>
                            </div>
                            ...
                            <div class="form-group col-6">
                                ...
                                <button type="button" id="imageClient" class="d-none" data-pc-toggle="modal" data-pc-target="#mediaModal"></button>
                                <input type="text" class="form-control mt-2 d-none"  id="imagePathInput" value="" name="imagePath" accept="image/*" readonly>
                                <img src="{{asset('storage/' . $clients->image)}}" alt="img...." width="100px" class="mt-3">
                            </div>
                        </div>
            ...
            <div class="modal-header">
                ...
                <div class="row align-items-center">
                    <form class="col-9" id="uploadForm" action="{{ route('admin.media.store') }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <label class="btn btn-outline-secondary" for="imageClientUpload">
                            <i class="ti ti-upload me-2"></i>
                            {{__("Click to Upload")}}
                        </label>
                        <input type="client" id="imageClientUpload" name="imageClient[]" accept="image/*" hidden multiple>
                    </form>
...
<script>
    $(document).ready(function() {
        ...
        $('#imageClientUpload').on('change', function() {
            // إنشاء كائن FormData لتضمين الملفات
            var formData = new FormData();
            formData.append('_token', "{{ csrf_token() }}");
            formData.append('imageClient', $(this).prop('clients')[0]);
            $.ajax({
