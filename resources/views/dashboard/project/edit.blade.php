                    ...
                    <form action="{{route('admin.project.update',$projects->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="form-group col-6 mb-3">
                                <x-form.input name="name_ar" label="{{__('Name_AR')}}" type="text" placeholder="{{__('enter name of projects in arabic')}}" required :value="$projects->name_ar" />
                            </div>
                            <div class="form-group col-6 mb-3">
                                <x-form.input name="name_en" label="{{__('Name_EN')}}" type="text" placeholder="{{__('enter name of projects in english')}}" required :value="$projects->name_en"/>
                            </div>
                            ...
                            <div class="form-group col-6">
                                ...
                                <img src="{{asset('storage/' . $projects->image)}}" alt="img...." width="100px" class="mt-3">
                            </div>

                            <div>
                                <label for="major">Select Specializations</label>
                                <select name="major[]" id="major" multiple required>
                                    @foreach($major as $major)
                                        <option value="{{ $major->id }}"
                                            @if($project->major->contains($major->id)) selected @endif>
                                            {{ $major->name_en}}
                                        </option>
                                    @endforeach
                                </select>
